"""Inference pipeline for rover hazard and navigation prediction."""
...
from ..data.dataset import collate_dict_batch, RoverDataset
from ..models.heads import OUTPUT_KEYS
from ..models.rover_navigation_net import RoverNavigationNet
from .checkpoint import load_checkpoint
from .config import PredictConfig

MAP_FILENAMES = {
    "traversability_score": "traversability_map.npy",
    "crater_hazard_probability": "crater_hazard_map.npy",
    "boulder_hazard_probability": "boulder_hazard_map.npy",
    "slope_hazard_score": "slope_hazard_map.npy",
    "navigation_cost_map": "navigation_cost_map.npy",
    "rover_safety_score": "rover_safety_map.npy",
}


class InferencePipeline:
    """Load checkpoints and produce rover traversability, hazard, and cost maps."""

    def __init__(self, config: PredictConfig) -> None:
        self.config = config
        self.device = torch.device(config.device)
        self.model = RoverNavigationNet().to(self.device)
    ...
    def _export_predictions(self, predictions: dict[str, np.ndarray]) -> None:
        ...
        summary = {
            ...
            "files": {key: MAP_FILENAMES[key] for key in predictions.keys()},
        }
        with (output_dir / "summary.json").open("w", encoding="utf-8") as handle:
            json.dump(summary, handle, indent=2)

        for key, array in predictions.items():
            filename = MAP_FILENAMES[key]
