"""Training loop for landing site intelligence."""
...
from Model_3_Landing_Site_Intelligence.src.data.dataset import LandingSiteDataset, collate_dict_batch
...
from Model_3_Landing_Site_Intelligence.src.training.losses import LandingSiteLoss
from Model_3_Landing_Site_Intelligence.src.training.metrics import MetricsCalculator, format_metrics
...
class Trainer:
    """Production-style trainer with validation, checkpointing, and early stopping."""

    def __init__(self, config: TrainingConfig) -> None:
        self.config = config
        set_seed(config.seed)
        self.device = torch.device(config.device)
        self.model = LandingSiteNet().to(self.device)
        self.criterion = LandingSiteLoss(
            slope_penalty_weight=config.slope_penalty_weight,
    ...
    def train_one_batch(self) -> dict[str, float]:
        ...
        dataset = LandingSiteDataset(num_samples=self.config.batch_size, patch_size=self.config.patch_size)
